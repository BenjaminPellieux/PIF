...
WasteDetection::WasteDetection(VideoSender& videoSender) : videoSender_(videoSender) {
    ...
    this->detect_pub = nh.advertise<geometry_msgs::Point>("Waste/Pos", 1000);
    this->vector_waste = nh.advertise<geometry_msgs::QuaternionStamped>("Waste/Geometry", 10000);
...
void WasteDetection::run(){
    ...
    while(ros::ok()) {
        ...
        for (size_t i = 0; i < contours.size(); i++) {
            ...
            if ((rect.y >= this->mid_height) && ((area > MIN_AREA) && (area < MAX_AREA))){ // Filtre basé sur la taille de l'objet
