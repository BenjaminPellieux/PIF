...
int MedianFiltring(EdIMAGE *image, EdIMAGE *imres)
{
  ...
  int	i,j;                             
  unsigned char tab_I[9];
  unsigned char cmpflag, tmp;
           ...
           POINT_X(point)++)
  {
    ...
    for(i = 0; i < 3; i++)
    {
      ...
      tab_I[3 * j + i] = PIXEL(image, pointv);
    } /* --- End of the Neighbourhood Video Scan --- */

    do{
	    cmpflag = 0;
	    for(unsigned char k = 0; k!= 8; k++){
		if(tab_I[k] > tab_I[k + 1]){
			tmp = tab_I[k + 1];
			tab_I[k + 1] = tab_I[k];
			tab_I[k] = tmp;
			cmpflag = 1;
